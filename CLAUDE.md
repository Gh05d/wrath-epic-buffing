...
- **Fetching in-game logs:** run `/check-logs` (user-invoked skill) to tail + filter the Steam Deck `Player.log` for mod-related exceptions after a deploy/repro.
- **"Ich blicke bei der UI nicht durch / wo fange ich an?"**: Seit 2026-08 gibt es einen Getting-Started-Artikel auf der Modseite (Articles-Tab) + Quickstart oben in der Description. Repo-Quellen: `docs/nexus-article-getting-started.bbcode`, `docs/nexus-description.bbcode`, Antwort-Template `docs/nexus-reply-getting-started.txt`. Bei solchen Reports: freundlich dorthin verweisen (jeder Wrath-Neuling ist anfangs überfordert); deckt der Artikel die Frage nicht ab, den Artikel erweitern statt einer Einzelantwort. It greps `Player.log` for `BuffIt2TheLimit|Exception|Error|…` over SSH (`deck-direct`).
...
- **"AoE-Party-Buff (z.B. Zephyr's Fleetness aus Expanded Content) meldet Erfolg, Slot weg, aber niemand bekommt den Buff"**: Fixed in v1.19.1. Mass-Buffs targeten den Caster; bei Spells mit `CanTargetSelf=false` brach `RuleCastSpell.OnTrigger` den Cast ab, NACHDEM der Mod den Slot gespent hatte. Jetzt wählt `ResolveMassTarget` ein gültiges Zentrum und der Handler bricht vor jedem Spend ab (Details: `claude-context/gotchas-casting.md`). Bei Reports mit älterer Version: Update empfehlen. Diagnose-Signatur im Player.log der alten Version: `Invalid target ... for spell '...'`.
- **"Reuse Magic Device (Trickster-Trick): Wand-Casts über den Mod verbrauchen trotzdem Charges"**: Fixed in v1.19.2. Der Mod dekrementierte Equipment-Charges manuell (`Charges--`) und umging damit den nativen Spend-Pfad `ItemEntity.SpendCharges(user)`, der Wands bei gesetztem `TricksterUseMagicDeviceUnlimitedWands`-Flag verschont (Details: `claude-context/gotchas-casting.md`). Wichtig: Der Trick ist pro Charakter — nur Casts, deren Caster den Trick hat (= der Trickster-Main), sparen die Charge; Wand-Casts anderer Partymitglieder verbrauchen weiterhin. Die Planungs-Credits pro Routine-Lauf bleiben auf die aktuelle Charge-Zahl gedeckelt. Bei Reports mit älterer Version: Update empfehlen.
- **"Infuse Magic Device (Trickster-Trick): Item-Casts nutzen das Basis-CL des Items statt +Mythic Rank"**: Fixed in v1.20.1. Der Mod baut für Scroll-/Potion-/Wand-Casts ein synthetisches `AbilityData` ohne `SourceItem` → die Engine nimmt nicht `GetParamsFromItem`, sondern den generischen Params-Pfad und übernimmt `OverrideCasterLevel` unverändert. Betroffen war nicht nur der Trickster-Boost, sondern auch Scroll Savant / Enhance Potion / `UnitPartItemCasterLevelBonus` (Details: `claude-context/gotchas-scanning.md`). Bei Reports mit älterer Version: Update empfehlen. Noch offen (kein Report): Item-Metamagic + ExtendPotion.
- **"„Limit casts to"-Buttons im Caster-Popout reagieren nicht (wenn viele Caster den Spell können)"**: Fixed in v1.20.2 (Nexus-Report mit Video). Nicht die Caster-Zahl war die Ursache, sondern die Popout-Höhe: bei Multi-Spellbook-Castern kommen „Source" + „Ban all sources" dazu, das Popout wächst nach oben, und seine obersten Zeilen rutschen unter das Summary-Header-Band (Normal/Quick/Important). Dessen Labels hängen als spätere Root-Geschwister über allem und haben `raycastTarget = true` (Gruppen-Tooltips seit v1.15.0) bei `cellSize = 400x100` — also ein breites unsichtbares Klick-Band. Diagnose-Signatur: Hover über der „Limit casts to"-Zeile öffnet das **Gruppen-Tooltip** statt zu klicken. Jetzt hängt das Popout via `PlaceCasterPopout` als Overlay auf Root-Ebene und wird ins Fenster geklemmt (Details: `claude-context/gotchas-ui.md`). Bei Reports mit älterer Version: Update empfehlen.
- **"Shifter's Fury: Parent-Icon ist da, tut aber nichts / kein Icon für Bite bzw. Claws"**: Fixed in v1.21.0 (Nexus-Report). `IsEffectivelyOn` las `ShiftersFuryPart.m_IsOn` als „Fury läuft" — das ist aber der `EntityPart`-Lifecycle-Flag und für jede anwesende Unit `true`, also galt der Eintrag immer als erledigt (Details: `claude-context/gotchas-scanning.md`). Jetzt via `HasFuryWeapon`. Getrennte Icons pro Waffe sind **nicht** möglich (alle Waffen-Facts stammen aus derselben Blueprint `b4f41266…`, der `BuffKey` ist die Blueprint-GUID) — stattdessen „Preferred fury weapon"-Zeile im Caster-Popout (nur ab ≥2 Waffenarten sichtbar, `SavedBufferState.FuryWeaponPreference`). Dass das Spiel Fury beim Rasten abschaltet, ist Vanilla: der Aspekt-Buff fällt weg → `OnPolymorphDeactivated` → Waffenliste wird neu gebaut, alle Einträge aus. Bei Reports mit älterer Version: Update empfehlen.
- **"Goggles of Pure Sight: Charge wird vom Mod nicht verbraucht, manuell neu castbar"**: Kein Bug (analysiert 2026-07-21). Die Brillen-Ability applied einen PERMANENTEN Buff („take 20 on next dispel magic attempt", ModifyD20), der erst beim nächsten Dispel-Wurf verbraucht wird. Solange er aktiv ist, skippt der Mod den Cast korrekt (Present-Check) → Charge bleibt. Manuelles Casten umgeht den Present-Check. Der Equipment-Charge-Verbrauchspfad selbst ist Ende-zu-Ende verifiziert (`EnsureFinalized` → `Charges--`, gated auf `ActuallyFired`; `IsSpendCharges` = `GainAbility && SpendCharges`, IL-geprüft). Nur falls ein Report ein echtes „applied" mit frischem Buff UND unveränderter Charge belegt: Player.log anfordern (`Consuming item after cast`-Errors).
