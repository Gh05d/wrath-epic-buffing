...
- **Cast on Combat Start** — Mark buffs to fire automatically when combat begins, or flip the global "Cast all buffs on combat start" switch in the settings panel to fire every configured buff without marking each one. Optional Skip-Animations mode casts instantly so you don't lose the surprise round to animation time.
